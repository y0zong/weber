...
export const c = (n, m, l) => n.setAttribute(m, l)
...
export function Router(rules, path) {
    ...
    const fragment = node => {
        ...
        node.tree.forEach(self => fg.appendChild(self))
        return fg
    }
        ...
        for (let i = 0, len = target.length; i < len; i++) {
            ...
            if (target_index == pre_route_target_index) {
                ...
                if (len - i == 1 && len_target) {
                    len_target.unmount(node => { node.tree.childNodes.forEach(self => parent.removeChild(self)) })
            ...
            if (pre_route_target_compt) {
                ...
                active_target_stack[i].unmount(node => {
                    node.tree.forEach(self => {
                        console.log(self)
                        parent.removeChild(self)
                    })
                })
                active_target_stack.splice(i)
                compt.mount(node => { parent.appendChild(fragment(node)) })
            } else {
                let parent = active_target_stack[i - 1]?.node || root
                console.log(pre_route_target_compt, i, parent)
                compt.mount(node => { 
                    if (parent.tree) {
                        parent.tree[0].parentNode.appendChild(fragment(node))
                    } else {
                        parent.appendChild(fragment(node))
                    }
                 })
            }
...
export function create(doms) {
    ...
    while (doms[index]) {
        ...
        if (item[2]) {
            ...
            if (item[3]) {
                item[3].map(prop => {
                    c(result.list[id], prop[0], prop[1])
                })
            }
        } else {
